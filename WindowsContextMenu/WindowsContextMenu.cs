...
public class WindowsContextMenu(ILogger? logger = null)
{
    ...
    public bool IsContextMenuExists(string fileExtension, string menuName)
    {
        ...
        try
        {
            RegistryKey? key = Registry.CurrentUser.OpenSubKey($@"Software\Classes\{fileExtension}\shell", true);
            if (key != null)
            {
                RegistryKey? subKey = key.OpenSubKey(menuName);
                return subKey != null;
            }
        }
        catch (Exception? ex)
    ...
    public void AddContextMenu(string fileExtension, string menuName, string menuCommand, string iconPath)
    {
        ...
        try
        {
            ...
            RegistryKey newKey = key.CreateSubKey(menuName);
